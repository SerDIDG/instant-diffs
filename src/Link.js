...
class Link {
    ...
    render() {
        ...
        // request | basic | event | none | link (deprecated) | default (deprecated)
        this.manual.behavior = this.node.dataset.instantdiffsLink;
        if ( this.manual.behavior === 'link' ) {
            this.manual.behavior = 'event';
        }
        if ( this.manual.behavior === 'default' ) {
            this.manual.behavior = 'request';
        }
        if ( [ 'request', 'basic', 'event', 'none' ].includes( this.manual.behavior ) ) {
            this.manual.hasLink = true;
            this.options.behavior = this.manual.behavior;
        }

        // Check if a link was marked manually by the "data-instantdiffs-alt-title" attribute
        // to show an original title instead of processed
        this.manual.isAltTitle = this.node.dataset.instantdiffsAltTitle === 'true';
        if ( this.manual.isAltTitle ) {
            this.options.showAltTitle = true;
        }
