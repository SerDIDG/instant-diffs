 ...
 * @property {import('./Page').Page.Any} [initiatorPage] - A Page instance that initiated this link
...
class Link {
	...
	process() {
		...
		if ( utilsLink.isMWLink( this.node, id.config.selectors.mwLinkExclude ) ) return;
	...
	processMWOptions() {
		...
		this.mw.isContent = utilsLink.isMWLink( this.node, id.config.selectors.mwLinkContent );
		if ( this.mw.isContent ) {
			this.options.behavior = 'request';
		}

		// Check if a link generated by the MediaWiki interface.
		...
		this.mw.hasLink = utilsLink.isMWLink( this.node, id.config.selectors.mwLink );
		if ( this.mw.hasLink ) {
			this.options.behavior = 'basic';

			this.mw.isDiffOnly = utilsLink.isMWLink( this.node, id.config.selectors.mwLinkDiffOnly );
			this.mw.isRevisionOnly = utilsLink.isMWLink( this.node, id.config.selectors.mwLinkRevisionOnly );

			this.mw.isPrepend = utilsLink.isMWLink( this.node, id.config.selectors.mwLinkPrepend );
			if ( this.mw.isPrepend ) {
				this.options.insertMethod = 'insertBefore';
			}

			this.mw.isAltTitle = utilsLink.isMWLink( this.node, id.config.selectors.mwLinkAltTitle );
			...
			if ( this.mw.line ) {
				...
				if ( utils.isEmpty( this.article.get( 'title' ) ) ) {
					const title = utilsLink.getMWLineTitle( this.mw.line );
					this.article.set( { title } );
				}
			}

			// Check if a link contributed by a user is in a commit message, etc.
			this.mw.isContentInside = utilsLink.isMWLink( this.node, id.config.selectors.mwLinkContentInside );
	...
	/**
	 * Checks whether the link is a valid diff or revision link.
	 * @return {boolean}
	 */
	get isValid() {
		return this.article?.isValid ?? false;
	}

	/**
	 * Checks whether the link points to a hidden diff or revision link.
	 * @return {boolean}
	 */
	get isHidden() {
		return this.article?.isHidden ?? false;
	}

	/**
	 * Checks whether the link points to a foreign wiki.
	 * @return {boolean}
	 */
	get isForeign() {
		return this.article?.isForeign ?? false;
	}
